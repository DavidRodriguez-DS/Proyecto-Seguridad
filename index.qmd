...
packages <- c(
  ...
  "caret", "rlang", "randomForest", "party", "ggforce", "randomForest", "tidyverse", "janitor", "party", "corrplot", "randomForestExplainer", "shiny", "nFactors", "GPArotation", "psych", "quarto", "scales"
)
...
```{r pre-procesamiento-2.1, echo=FALSE, warning=FALSE}
# Gráfico de barras

ggplot(enusc_percepcion, aes(x = variable, fill = factor(valor, levels = c(3, 2, 1)))) +
  geom_bar(position = "fill", color = "black") +
  scale_fill_manual(
    values = alpha(c("darkred", "darkgray","black"), 0.8),  # Se aplica el alpha correctamente
...
for (i in 1:total_pages) {
  plot <- ggplot(data_long, aes(x = as.factor(valor), y = percentage)) + 
    geom_bar(stat = "identity", fill = alpha("steelblue", 0.8), color = "black") +
...
2.  *Identidad de génerp ("rph_idgen")*:
...
3.  *Pertenencia indígena ("rph_pertenencia_indigena")*: 

    Rango: 0 (No pertenece) a 1 (Pertenece). Distribución: La media (0,09) indica que menos del 10% de los encuestados pertenece a un pueblo indígena, que se condice con resultados arrojados por instrumentos como CENSO 0217 y encuestas de opinión pública, las que arrojan que alrededor del 10% de nuestra población se identifica como perteneciente a un pueblo indígena..

4.  *Nacionalidad ("rph_nacionalidad")*: 

    Rango: 1 (Chilena) a 2 (Chilena y/u otra). Distribución: La mediana (1,0) y la media (1,06) reflejan que la mayoría de los encuestados se identifica exclusivamente como chilenos.

5.  *Nivel educacional ("rph_nivel")*:
...
6.  *Situación laboral actual ("rph_situacion_laboral_a")*:
...
8.  *Nivel socioeconómico ("rph_nse")*:
...
enusc_reducida %>%
  ...
  ggplot(aes(x = valor)) +
  geom_histogram(binwidth = 0.25, fill = alpha("steelblue", 0.8), color = "black") +
...
```{r t-test-2.1, echo=FALSE, warning=FALSE}
...
```{r anova-1.3, echo=FALSE, warning=FALSE}
...
```{r anova-1.4, echo=FALSE, warning=FALSE}
...
```{r anova-1.5, echo=FALSE, warning=FALSE}
...
```{r anova-2.2, echo=FALSE, warning=FALSE}
...
```{r anova-2.3, echo=FALSE, warning=FALSE}
...
```{r anova-2.4, echo=FALSE, warning=FALSE}
...
```{r anova-2.5, echo=FALSE, warning=FALSE}
...
```{r pca.1, echo=FALSE, warning=FALSE}
...
```{r varianza-pca.1, echo=FALSE, warning=FALSE}
...
```{r varianza-pca.2, echo=FALSE, warning=FALSE}
...
ggplot(pca_cargas_long %>% filter(Componente %in% c("Percepción General de Inseguridad", "Inseguridad en Lugares Habituales")), 
  ...
  labs(title = "Cargas de las Variables en Componentes Principales",
       ...
       y = "Carga",
       fill = "Componente") +
  scale_fill_manual(values = c("Percepción General de Inseguridad" = "darkred", 
                              "Inseguridad en Lugares Habituales" = "steelblue")) +
  theme_minimal()
...
```{r regresion-lineal.1, echo=FALSE, warning=FALSE}
...
```{r modelo-predictivo, echo=FALSE, warning=FALSE}
...
<u><strong>Resumen del modelo</strong></u>:
...
## Clusterización: Identificación de perfiles
...
ggplot(data_model, aes(x = inseguridad_general, y = inseguridad_habituales, color = etiqueta_cluster)) +
  ...
  scale_color_manual(values = c("darkred", "firebrick")) +  # Asigna el color darkred a todos los puntos
  theme_minimal()
...
Caracterizamos a nuestros conglomerados:
...
ggplot(promedios_nse_cluster, aes(x = as.factor(rph_nse), y = promedio_inseguridad_general, fill = etiqueta_cluster)) +
  ...
  scale_fill_manual(values = alpha(c("darkred", "steelblue"), 0.85)) +
  theme_minimal()
