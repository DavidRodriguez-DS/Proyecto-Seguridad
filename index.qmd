...
```{r seteo-paquetes-y-renv, results='hide'}
rm(list = ls())
...
```{r matriz-correlacion-inseguridad}
...
```{r visualizacion-variables-sociodemo}
...
```{r matriz-correlacion-sociodemo-inseguridad}
...
```{r matriz-correlacion-sociodemo-inseguridad-2}
...
###Resumen del modelo para evaluar el ajuste y la relevancia de las variables.
...
rph_nivel: Los niveles educativos más altos (rph_nivel2 y rph_nivel3) están asociados con mayor percepción de inseguridad en comparación con el nivel de referencia (nivel 0: "Nunca asistió"). Esto podría reflejar que mayores niveles educativos están relacionados con mayor consciencia sobre los riesgos.
...
rph_situacion_laboral_a: Las personas empleadas (1) perciben una mayor inseguridad general (coeficiente positivo de 0.4493). Esto podría estar relacionado con su exposición a situaciones de inseguridad al trasladarse o trabajar fuera del hogar.
...
rph_edad: Relación negativa significativa (-0.0434). A mayor edad, menor es la percepción de inseguridad en lugares habituales. Esto puede reflejar que personas mayores se sienten más seguras en sus espacios privados.

rph_nivel3: Personas con educación superior tienen una percepción significativamente mayor de inseguridad en lugares habituales en comparación con quienes no asistieron a la escuela (coeficiente 0.290).

rph_nse: A medida que el nivel socioeconómico aumenta (rph_nse2 y rph_nse3), la percepción de inseguridad habitual también aumenta. Esto puede deberse a que personas de niveles socioeconómicos altos son más conscientes de los riesgos o están expuestas a situaciones específicas que aumentan su percepción de inseguridad.

rph_situacion_laboral_a1: Las personas empleadas tienen una percepción ligeramente mayor de inseguridad en lugares habituales (coeficiente 0.0393).

rph_pertenencia_indigena1: Pertenecer a un pueblo indígena se asocia con una menor percepción de inseguridad habitual (-0.0974). Esto podría estar relacionado con diferencias culturales o contextos específicos donde habitan estas personas.

rph_nacionalidad2: Las personas extranjeras o con doble nacionalidad perciben menos inseguridad habitual (-0.1014).

```{r regresion-lineal-3}
...
cluster_summary %>%
  kable(
    format = "markdown", # Cambiar a "html" o "latex" según necesidad
    col.names = c(
      "Cluster",
      "Cantidad",
...
```
Cluster 1: Según los datos, vemos que el cluster 1 tiene una menor media de inseguridad general que el promedio (-1.446). Respecto a la inseguridad habitual, vemos también que esta es menor al promedio moderadamente (-0.191). Respecto a la espera observamos que tienen una inseguridad moderada respecto a la espera (0.388) y una tasa muy leve de inseguridad respecto al transporte (0.009). El cluster tiene un total de 28.541 observaciones.

Cluster 2: En este cluster vemos una inseguridad general mayor a la media (1.940). Del mismo modo, observamos que su media habitual también es mayor al promedio con un 0.256. No obstante, en la espera tienen una media de inseguridad menor al promedio (-0520)y una leve inseguridad respecto a la media del transporte (-0.012). El cluster tiene un total de 21.272 observaciones.

```{r clustering-visualización}
...
Como podemos observar, la diferencia entre ambos cluster ronda en torno a la inseguridad general. Mientras que el cluster 1 tiene inseguridad más concentrada hacia la percepción de inseguridad en general de manera negativa, el cluster 2 se concentra hacia una media mayor al promedio en lo que respecta a la inseguridad en general. Siendo lo que diferencia a ambos grupos.
...
kable(caracterizacion_clusters, 
      col.names = c("Cluster", "Número de Observaciones", "Edad Promedio", 
                    ...
                    "Sin Asistencia Educacional (%)", "NSE Bajo (%)", 
                    "NSE Medio (%)", "NSE Alto (%)", "Activos Laboralmente (%)", 
...
```{r cluster-nse}
#Hacemos un resumen en torno a nivel-socioeconomico
