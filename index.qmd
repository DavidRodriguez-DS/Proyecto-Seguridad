...
# El número de clusters (centers) se eligió arbitrariamente como 2.
...
# Revisar la distribución de los clusters
table(data_model$etiqueta_cluster)
...
ggplot(data_model, aes(x = inseguridad_general, y = inseguridad_habituales, color = etiqueta_cluster)) +
  ...
  labs(
    ...
    color = "Etiqueta del Cluster"
  ) +
  theme_minimal()

```
```{r}
resumen_clusters <- data_model %>%
  group_by(etiqueta_cluster) %>%
  summarise(
    promedio_inseguridad_general = mean(inseguridad_general),
    promedio_inseguridad_habituales = mean(inseguridad_habituales),
    cantidad_personas = n()
  )
...
caracterizacion_clusters <- data_model %>%
  ...
  summarise(
    ...
    sin_asistencia_educacional = mean(rph_nivel == 0, na.rm = TRUE) * 100,
...
```{r}
# Calcular los promedios de inseguridad general por NSE y cluster
...
ggplot(promedios_nse_cluster, aes(x = as.factor(rph_nse), y = promedio_inseguridad_general, fill = etiqueta_cluster)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Promedio de Inseguridad General por NSE y Cluster",
